                ...
                try
                {
                    ...
                    var arguments =
                        string.Format(
                            ...
                            "-hide_banner -loglevel error -y -i \"{0}\" -vn -c:a aac -b:a 128k -ar 44100 -ac 2 -f hls -hls_time 1 -hls_list_size 6 -hls_flags delete_segments+omit_endlist+independent_segments -hls_segment_filename \"{1}\" \"{2}\"",
                            inputPath,
                            segmentPattern,
                            playlistPath);
